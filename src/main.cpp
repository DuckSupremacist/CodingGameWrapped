...
void printFromGridT(grid_t grid)
{
    std::cerr << "Depth: " << (grid >> DEPTH_SHIFT) << std::endl;
    std::cerr << "Grid:" << std::endl << "\t";
    for (size_t i = 0; i < GRID_SIZE; i++)
    {
        std::cerr << std::dec << ((grid >> (i * TILE_GRID_OFFSET)) & LOOSE_TILE_MASK) << " ";
...
void printFromRawHashT(raw_hash_t hash)
{
    std::cerr << "Hash:" << std::endl << "\t";
    for (size_t i = 0; i < GRID_SIZE; i++)
    {
        std::cerr << std::dec << hash[i] << " ";
        if (i % GRID_SIDE_SIZE == 2)
        {
            std::cerr << std::endl << "\t";
        }
...
class Transformer
{
  protected:
    ...
    static constexpr std::array<Transformation, 8> TRANSFORMS = {
        Transformation::ID,
        Transformation::H_FLIP,
        Transformation::V_FLIP,
...
class KeyTransformer : public Transformer
{
  public:
    ...
    {
        uint64_t transformed_key = key;
        if (static_cast<TransformOperations>(t) & H)
        {
            transformed_key =
                ((key & 0b11111'11111'11111'00000'00000'00000'00000'00000'00000) >> (TILE_GRID_OFFSET * 6)) |
                (key & 0b00000'00000'00000'11111'11111'11111'00000'00000'00000) |
                ((key & 0b00000'00000'00000'00000'00000'00000'11111'11111'11111) << (TILE_GRID_OFFSET * 6));
        }
        if (static_cast<TransformOperations>(t) & V)
        {
            transformed_key =
                ((key & 0b11111'00000'00000'11111'00000'00000'11111'00000'00000) >> (TILE_GRID_OFFSET * 2)) |
                (key & 0b00000'11111'00000'00000'11111'00000'00000'11111'00000) |
                ((key & 0b00000'00000'11111'00000'00000'11111'00000'00000'11111) << (TILE_GRID_OFFSET * 2));
        }
        if (static_cast<TransformOperations>(t) & T)
        {
            transformed_key =
                (key & 0b11111'00000'00000'00000'11111'00000'00000'00000'11111) |
                ((key & 0b00000'11111'00000'00000'00000'11111'00000'00000'00000) >> (TILE_GRID_OFFSET * 2)) |
                ((key & 0b00000'00000'00000'11111'00000'00000'00000'11111'00000) << (TILE_GRID_OFFSET * 2)) |
                ((key & 0b00000'00000'11111'00000'00000'00000'00000'00000'00000) >> (TILE_GRID_OFFSET * 4)) |
                ((key & 0b00000'00000'00000'00000'00000'00000'11111'00000'00000) << (TILE_GRID_OFFSET * 4));
        }
    ...
    static constexpr std::pair<hash_key_t, Transformation> canonical(hash_key_t key)
    {
        ...
        if (found == 1)
        {
            Transformation transposed = static_cast<Transformation>(static_cast<TransformOperations>(transformation) |
                                                                    T);
...
class RawHashTransformer : public Transformer
{
  public:
    static constexpr raw_hash_t transform(raw_hash_t raw_hash, Transformation t)
    {
        ...
        if (static_cast<TransformOperations>(t) & H)
        {
            transformed_hash =
                {
                    transformed_hash[6], transformed_hash[7], transformed_hash[8],
                    transformed_hash[3], transformed_hash[4], transformed_hash[5],
                    transformed_hash[0], transformed_hash[1], transformed_hash[2],
                };
        }
        if (static_cast<TransformOperations>(t) & V)
        {
            transformed_hash =
                {
                    transformed_hash[2], transformed_hash[1], transformed_hash[0],
                    transformed_hash[5], transformed_hash[4], transformed_hash[3],
                    transformed_hash[8], transformed_hash[7], transformed_hash[6],
                };
        ...
        if (static_cast<TransformOperations>(t) & T)
        {
            transformed_hash =
                {
                    transformed_hash[0], transformed_hash[3], transformed_hash[6],
                    transformed_hash[1], transformed_hash[4], transformed_hash[7],
                    transformed_hash[2], transformed_hash[5], transformed_hash[8],
                };
        }
  ...
  public:
    std::optional<raw_hash_t> retrieve(hash_key_t key)
    {
        ...
        if (it != _hash_lookup.end())
        {
            ...
            printFromGridT(key);
            printFromRawHashT(it->second);
            std::cerr << "Transformed hash:" << std::endl;
            std::cerr << "Transformation: " << static_cast<int>(t) << std::endl;
            printFromGridT(transformed_key);
            printFromRawHashT(_hash_lookup[transformed_key]);
#endif
            return RawHashTransformer::reverseTransform(it->second, t);
        }
        return std::nullopt;
    }
    ...
    {
        auto [transformed_key, t] = KeyTransformer::canonical(key);
        _hash_lookup[transformed_key] = RawHashTransformer::transform(hash, t);
#ifdef BUILD_TESTS
        statistics.states_stored_in_hash_map++;
        std::cerr << "Stored hash for key:" << std::endl;
        printFromGridT(key);
        printFromRawHashT(hash);
        std::cerr << "Transformed hash:" << std::endl;
        std::cerr << "Transformation: " << static_cast<int>(t) << std::endl;
        printFromGridT(transformed_key);
        printFromRawHashT(_hash_lookup[transformed_key]);
